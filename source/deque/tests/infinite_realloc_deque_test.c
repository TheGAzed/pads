...
TEST IRD_DESTROY_03(void) {
    ...
    for (size_t i = 0; i < REALLOC_DEQUE_CHUNK - 1; ++i) {
...
TEST IRD_DESTROY_04(void) {
    ...
    for (size_t i = 0; i < REALLOC_DEQUE_CHUNK; ++i) {
        enqueue_front(&test, (DEQUE_DATA_TYPE) { .sub_one = i, });
    }
...
SUITE (infinite_realloc_deque_test) {
    ...
    // forevery
    RUN_TEST(IRD_FOREVERY_01); RUN_TEST(IRD_FOREVERY_02); RUN_TEST(IRD_FOREVERY_03); RUN_TEST(IRD_FOREVERY_04);
    RUN_TEST(IRD_FOREVERY_05); RUN_TEST(IRD_FOREVERY_06); RUN_TEST(IRD_FOREVERY_07); RUN_TEST(IRD_FOREVERY_08);
}
