...
#define INFINITE_ALLOCATED_DOUBLE_LIST  51
#define FINITE_ALLOCATED_DOUBLE_LIST    52
#define INFINITE_REALLOC_DOUBLE_LIST    53
#define FINITE_PRERPOCESSOR_DOUBLE_LIST 54
...
#define IS_INFINITE_DOUBLE_LIST (DOUBLE_LIST_MODE & 0x01)
...
typedef DOUBLE_LIST_DATA_TYPE (*copy_list_fn)    (const DOUBLE_LIST_DATA_TYPE);
typedef void           (*destroy_list_fn) (DOUBLE_LIST_DATA_TYPE *);
typedef int            (*compare_list_fn) (const void *, const void *);
typedef bool           (*operate_list_fn) (DOUBLE_LIST_DATA_TYPE *, void *);
...
static inline double_list_s create_list(void) {
    return (double_list_s) { 0 };
}

static inline void destroy_list(double_list_s * list, const destroy_list_fn destroy) {
...
static inline void insert_at_list(double_list_s * list, const size_t index, const DOUBLE_LIST_DATA_TYPE element) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter is NULL.");
    ...
    struct list_node * temp = DOUBLE_LIST_ALLOC(sizeof(struct list_node));
    DOUBLE_LIST_ASSERT(temp && "[ERROR] Memory allocation failed.");
    memcpy(&(temp->element), &element, sizeof(DOUBLE_LIST_DATA_TYPE));
...
static inline DOUBLE_LIST_DATA_TYPE remove_first_list(double_list_s * list, const DOUBLE_LIST_DATA_TYPE element, const compare_list_fn compare) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter is NULL.");
    DOUBLE_LIST_ASSERT(list->size && "[ERROR] Can't remove from empty double_list.");
    DOUBLE_LIST_ASSERT(list->head && "[ERROR] Can't remove from empty double_list.");

    struct list_node ** current = &(list->head);
    for (size_t s = 0; s < list->size; ++s) {
        const int comparison = compare ? compare(&((*current)->element), &element) : memcmp(&((*current)->element), &element, sizeof(DOUBLE_LIST_DATA_TYPE));
        if (0 == comparison) {
...
static inline DOUBLE_LIST_DATA_TYPE remove_last_list(double_list_s * list, const DOUBLE_LIST_DATA_TYPE element, const compare_list_fn compare) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter is NULL.");
    DOUBLE_LIST_ASSERT(list->size && "[ERROR] Can't remove from empty double_list.");
    DOUBLE_LIST_ASSERT(list->head && "[ERROR] Can't remove from empty double_list.");
...
static inline DOUBLE_LIST_DATA_TYPE remove_at_list(double_list_s * list, const size_t index) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter is NULL.");
    DOUBLE_LIST_ASSERT(list->size && "[ERROR] Can't remove from empty double_list.");
    DOUBLE_LIST_ASSERT(list->head && "[ERROR] Can't remove from empty double_list.");
    ...
    if (index < (list->size >> 1)) { // special case when index is 0 so head must change to next
        ...
        temp = *current;
        *current = (*current)->next;
    } else { // head might change if size is one, but since the double_list is circular temp will point to head after loop
        temp = list->head;
        for (size_t i = 0; i < list->size - index; ++i) {
            temp = temp->prev;
        }
...
static inline void reverse_list(double_list_s * list) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter is NULL.");
...
static inline void shift_next_list(double_list_s * list, const size_t shift) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter is NULL.");
...
static inline void shift_prev_list(double_list_s * list, const size_t shift) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter is NULL.");
...
static inline double_list_s splice_list(double_list_s * restrict list_one, double_list_s * restrict list_two, const size_t index) {
...
static inline double_list_s split_list(double_list_s * list, const size_t index, const size_t size) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] List pointer is NULL");
    DOUBLE_LIST_ASSERT(index < list->size && "[ERROR] Can only split at index less than double_list size.");
    DOUBLE_LIST_ASSERT(size <= list->size && "[ERROR] Size parameter bigger than double_list size.");
...
static inline DOUBLE_LIST_DATA_TYPE get_list(const double_list_s list, const size_t index) {
    DOUBLE_LIST_ASSERT(list.head && "[ERROR] Can't get element from empty double_list.");
    DOUBLE_LIST_ASSERT(list.size && "[ERROR] Can't get element from empty double_list.");
    DOUBLE_LIST_ASSERT(index < list.size && "[ERROR] 'index' parameter exceeds double_list size.");
...
static inline bool is_empty_list(const double_list_s list) {
    return (list.size == 0);
}

static inline bool is_full_list(const double_list_s list) {
    return !(~list.size);
}

static inline double_list_s copy_list(const double_list_s list, const copy_list_fn copy) {
...
static inline void sort_list(double_list_s const * list, const sort_list_fn sort, const compare_list_fn compare) {
...
static inline void foreach_list(double_list_s const * list, const operate_list_fn operate, void * args) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter pointer is NULL.");
...
static inline void foreach_reverse_list(const double_list_s * list, const operate_list_fn operate, void * args) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter pointer is NULL.");
    DOUBLE_LIST_ASSERT(operate && "[ERROR] 'operate' parameter pointer is NULL.");
...
static inline double_list_s create_list(const size_t max) {
...
static inline double_list_s splice_list(double_list_s * restrict list_one, double_list_s * restrict list_two, const size_t index, const size_t max) {
    DOUBLE_LIST_ASSERT(list_one && "[ERROR] 'list_one' parameter pointer is NULL.");
    DOUBLE_LIST_ASSERT(list_two && "[ERROR] 'list_two' parameter pointer is NULL.");
    DOUBLE_LIST_ASSERT(max && "[ERROR] maximum size can't be zero.");
    DOUBLE_LIST_ASSERT(list_one->size + list_two->size <= max && "[ERROR] Combined lists' sizes exceed max size of new list.");
    DOUBLE_LIST_ASSERT(list_one != list_two && "[ERROR] Lists can't be the same.");
    DOUBLE_LIST_ASSERT(index <= list_one->size && "[ERROR] index can't exceed list_one's size");
    ...
    list_one->elements = DOUBLE_LIST_REALLOC(list_one->elements, sizeof(DOUBLE_LIST_DATA_TYPE) * max);
    list_one->node[NEXT_IDX] = DOUBLE_LIST_REALLOC(list_one->node[NEXT_IDX], sizeof(size_t) * max);
    list_one->node[PREV_IDX] = DOUBLE_LIST_REALLOC(list_one->node[PREV_IDX], sizeof(size_t) * max);
    ...
    DOUBLE_LIST_FREE(list_two->node[NEXT_IDX]);
    DOUBLE_LIST_FREE(list_two->node[PREV_IDX]);
...
static inline double_list_s split_list(double_list_s * list, const size_t index, const size_t size, const size_t max) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] List pointer is NULL");
...
static inline void sort_list(double_list_s * list, const sort_list_fn sort, const compare_list_fn compare) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter pointer is NULL");
...
static inline double_list_s splice_list(double_list_s * restrict list_one, double_list_s * restrict list_two, const size_t index) {
    ...
    list_one->node[NEXT_IDX] = DOUBLE_LIST_REALLOC(list_one->node[NEXT_IDX], sizeof(size_t) * alloc_size);
    list_one->node[PREV_IDX] = DOUBLE_LIST_REALLOC(list_one->node[PREV_IDX], sizeof(size_t) * alloc_size);
...
static inline void foreach_list(double_list_s * list, const operate_list_fn operate, void * args) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter pointer is NULL.");
    DOUBLE_LIST_ASSERT(operate && "[ERROR] 'operate' parameter pointer is NULL.");
...
static inline void foreach_reverse_list(double_list_s * list, const operate_list_fn operate, void * args) {
    DOUBLE_LIST_ASSERT(list && "[ERROR] 'list' parameter pointer is NULL.");
    DOUBLE_LIST_ASSERT(operate && "[ERROR] 'operate' parameter pointer is NULL.");
