...
#ifndef HASH_SET_VALUE_TYPE

/// @brief To change, use: #define HASH_SET_VALUE_TYPE [type].
#define HASH_SET_VALUE_TYPE size_t
...
typedef HASH_SET_DATA_TYPE  (*copy_hash_set_fn)    (const HASH_SET_DATA_TYPE);
/// @brief Function pointer to destroy/free an element for hash set element.
typedef void                (*destroy_hash_set_fn) (HASH_SET_DATA_TYPE *);
/// @brief Function pointer to hash a set element into a value.
typedef HASH_SET_VALUE_TYPE (*hash_set_fn)         (const HASH_SET_DATA_TYPE);
/// @brief Normalizes hash value into an index size in range [ 0 ... (HASH_SET_SIZE - 1) ]
typedef size_t              (*normal_hash_set_fn)  (const HASH_SET_VALUE_TYPE);
/// @brief Function pointer to operate on a single set element based on generic arguments.
typedef bool                (*operate_hash_set_fn) (HASH_SET_DATA_TYPE *, void *);
/// @brief Function pointer to manage an array of set elements based on generic arguments.
typedef void                (*manage_hash_set_fn)  (HASH_SET_DATA_TYPE *, const size_t, void *);
...
typedef struct hash_set {
    hash_set_fn hash;
    normal_hash_set_fn normalize;
    size_t next[HASH_SET_SIZE];
    size_t head[HASH_SET_SIZE];
    size_t size, empty;
    HASH_SET_DATA_TYPE elements[HASH_SET_SIZE];
    HASH_SET_VALUE_TYPE values[HASH_SET_SIZE];
} hash_set_s;

static inline hash_set_s create_hash_set(const hash_set_fn hash, const normal_hash_set_fn normalize) {
    HASH_SET_ASSERT(hash && "[ERROR] 'hash' parameter is NULL.");

    hash_set_s set = { .hash = hash, .size = 0, .empty = HASH_SET_SIZE, .normalize = normalize };
...
static inline hash_set_s copy_hash_set(const hash_set_s * set, const copy_hash_set_fn copy) {
    ...
    for (size_t i = 0; i < HASH_SET_SIZE; ++i) {
        ...
        for (size_t current = set->head[i]; HASH_SET_SIZE != current; current = set->next[current]) {
            ...
            memcpy(replica.values + free_index, set->values + current, sizeof(HASH_SET_VALUE_TYPE));
            replica.size++;
...
static inline bool contains_hash_set(const hash_set_s * set, const HASH_SET_DATA_TYPE element) {
    ...
    const HASH_SET_VALUE_TYPE element_hash = set->hash(element);
    const size_t element_modulo = set->normalize(element_hash);
...
static inline void insert_hash_set(hash_set_s * set, const HASH_SET_DATA_TYPE element) {
    ...
    for (size_t current = set->head[element_modulo]; HASH_SET_SIZE != current; current = set->next[current]) {
        HASH_SET_ASSERT(memcmp(&element_hash, set->values + current, sizeof(HASH_SET_VALUE_TYPE)) && "[ERROR] Element already in set.");
    }
...
static inline HASH_SET_DATA_TYPE remove_hash_set(hash_set_s * set, const HASH_SET_DATA_TYPE element) {
    ...
    // check if element is in set or not
    for (size_t * current = set->head + element_modulo; HASH_SET_SIZE != (*current); current = set->next + (*current)) {
        if (memcmp(&element_hash, set->values + (*current), sizeof(HASH_SET_VALUE_TYPE))) {
            continue;
        }
...
static inline hash_set_s union_hash_set(const hash_set_s * set_one, const hash_set_s * set_two, const copy_hash_set_fn copy) {
    ...
    for (size_t i = 0; i < HASH_SET_SIZE; ++i) {
        for (size_t current_two = set_two->head[i]; current_two != HASH_SET_SIZE; current_two = set_two->next[current_two]) {
            const HASH_SET_VALUE_TYPE hash_two = set_two->values[current_two];
            ...
            for (size_t current_union = union_set.head[i]; current_union != HASH_SET_SIZE; current_union = union_set.next[current_union]) {
                if (!memcmp(&hash_two, union_set.values + current_union, sizeof(HASH_SET_VALUE_TYPE))) {
                    contains_two = true;
                    break;
                }
            }
            ...
            if (!contains_two) { // if two's element is not contained in union push it into union
                ...
                union_set.values[free_index] = hash_two;
                union_set.size++;
...
static inline hash_set_s intersect_hash_set(const hash_set_s * set_one, const hash_set_s * set_two, const copy_hash_set_fn copy) {
    ...
    for (size_t i = 0; i < HASH_SET_SIZE; ++i) {
        for (size_t current_one = set_one->head[i]; current_one != HASH_SET_SIZE; current_one = set_one->next[current_one]) {
            const HASH_SET_VALUE_TYPE hash_one = set_one->values[current_one];

            // check if set two contains one's element
            bool contains_one = false;
            for (size_t current_two = set_two->head[i]; current_two != HASH_SET_SIZE; current_two = set_two->next[current_two]) {
                if (!memcmp(&hash_one, set_two->values + current_two, sizeof(HASH_SET_VALUE_TYPE))) {
                    contains_one = true;
                    break;
                }
            }
            ...
            if (contains_one) {
                ...
                intersect_set.values[free_index] = hash_one;
                intersect_set.size++;
    ...
    for (size_t i = 0; i < HASH_SET_SIZE; ++i) {
        for (size_t current_one = set_one->head[i]; current_one != HASH_SET_SIZE; current_one = set_one->next[current_one]) {
            ...
            if (!contains_one) {
                ...
                subtract_set.values[free_index] = hash_one;
                subtract_set.size++;
    ...
    for (size_t i = 0; i < HASH_SET_SIZE; ++i) {
        ...
        for (size_t current_one = set_one->head[i]; current_one != HASH_SET_SIZE; current_one = set_one->next[current_one]) {
            ...
            if (!contains_one) {
                ...
                exclude_set.values[free_index] = hash_one;
                exclude_set.size++;
        ...
        for (size_t current_two = set_two->head[i]; current_two != HASH_SET_SIZE; current_two = set_two->next[current_two]) {
            ...
            bool contains_two = false;
            for (size_t current_one = set_one->head[i]; current_one != HASH_SET_SIZE; current_one = set_one->next[current_one]) {
                if (!memcmp(&hash_two, set_one->values + current_one, sizeof(HASH_SET_VALUE_TYPE))) {
                    contains_two = true;
                    break;
                }
            }
            ...
            if (!contains_two) {
                ...
                exclude_set.values[free_index] = hash_two;
                exclude_set.size++;
...
static inline bool is_subset_hash_set(const hash_set_s * super, const hash_set_s * sub) {
    ...
    for (size_t i = 0; i < HASH_SET_SIZE; ++i) {
        for (size_t current_sub = sub->head[i]; current_sub != HASH_SET_SIZE; current_sub = sub->next[current_sub]) {
            const HASH_SET_VALUE_TYPE hash_sub = sub->values[current_sub];
            ...
            for (size_t current_super = super->head[i]; current_super != HASH_SET_SIZE; current_super = super->next[current_super]) {
                if (!memcmp(&hash_sub, super->values + current_super, sizeof(HASH_SET_VALUE_TYPE))) {
                    contains_sub = true;
                    break;
                }
            }
